...
std::optional<RoutingEntry> RoutingTable::getRoutingEntry(ip_addr ip) {
    RoutingEntry* best_entry = nullptr;
    int best_prefix = -1;
    for (auto &entry : routingEntries) {
        // Use the correct 'mask' field instead of 'netmask'
        if ((ip & entry.mask) == (entry.dest & entry.mask)) {
            int prefix_length = __builtin_popcount(entry.mask);
            if (prefix_length > best_prefix) {
